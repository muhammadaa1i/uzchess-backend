...
export class CreateBookHandler implements ICommandHandler<CreateBookCommand> {
    async execute(cmd: CreateBookCommand) {
        ...
        DoesNotExistException.ThrowIf(authorsCount !== cmd.authorIds.length, "One or more authors not found")
        ...
        const bookAuthors = cmd.authorIds.map((authorId) => BookAuthor.create({bookId: book.id, authorId}))
        await BookAuthor.save(bookAuthors)

        return plainToInstance(CreateBookResponse, {
            ...book,

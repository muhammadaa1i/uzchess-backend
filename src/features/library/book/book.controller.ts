...
export class BookController {
  ...
  async getAll(@Query() payload: GetBooksRequest) {
    return await this.queryBus.execute(new GetBooksQuery(payload));
  }
  ...
  @Public()
  @Get("top-rated")
  @ApiOkResponse({ type: [GetTopRatedBooksResponse] })
  async getTopRated() {
    return await this.queryBus.execute(new GetTopRatedBooksQuery());
  }
  ...
  async create(
    ...
    return await this.cmdBus.execute(
      new CreateBookCommand(payload, cover.path),
    );
  }
  ...
  async update(
    ...
    return await this.cmdBus.execute(
      new UpdateBookCommand(id, payload, cover?.path),
    );
  }

...
describe("GetFavouritesHandler", () => {
  ...
  it("maps books and rating stats correctly", async () => {
    jest.spyOn(Favourite, "findBy").mockResolvedValue([
      { bookId: 1, userId: 9 },
      { bookId: 2, userId: 9 },
    ] as any);
  ...
  it("returns an empty array when there are no favourites, without querying books or ratings", async () => {
    jest.spyOn(Favourite, "findBy").mockResolvedValue([]);

...
export class CreateSectionHandler implements ICommandHandler<CreateSectionCommand> {
  ...
  async execute(cmd: CreateSectionCommand) {
    const courseExists = await Course.existsBy({ id: cmd.payload.courseId });
    ...
    await Promise.all([
      this.cache.del(sectionsListCacheKey(cmd.payload.courseId)),
      this.cache.del(courseByIdCacheKey(cmd.payload.courseId)),

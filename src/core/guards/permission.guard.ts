...
export class PermissionGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext) {
        ...
        let userPermissions: any = await this.cache.get(`permission:${req.user.id}`)
        ...
        if (!userPermissions) {
            ...
            const rolePermissions = await Permission.findBy({rolePermissions: {role: {userRoles: {userId: req.user.id}}}})
